...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!isValidFullname(fullname)) {
                request.setAttribute("error", "Tên phải có ít nhất 2 ký tự, chỉ chứa chữ cái và dấu cách, chữ cái đầu viết hoa.");
            ...
            if (!isValidPassword(password)) {
                request.setAttribute("error", "Mật khẩu phải có ít nhất 8 ký tự và chữ cái đầu tiên phải viết hoa.");
                request.getRequestDispatcher("/register.jsp").forward(request, response);
                return;
            }
            ...
            if (!isValidPhone(phone)) {
                request.setAttribute("error", "Số điện thoại phải có đúng 10 chữ số và bắt đầu bằng 0.");
                request.getRequestDispatcher("/register.jsp").forward(request, response);
                return;
            }
            ...
            if (!isValidAddress(address)) {
                request.setAttribute("error", "Địa chỉ phải có ít nhất 5 ký tự và không được để trống.");
                request.getRequestDispatcher("/register.jsp").forward(request, response);
                return;
            }
            ...
            if (!isValidBirthday(birthday)) {
                request.setAttribute("error", "Người dùng phải từ 16 tuổi trở lên.");
                request.getRequestDispatcher("/register.jsp").forward(request, response);
                return;
            }
    ...
    private boolean isValidBirthday(Date birthday) {
        ...
        LocalDate currentDate = LocalDate.now(); // Ngày hiện tại: 21/05/2025
